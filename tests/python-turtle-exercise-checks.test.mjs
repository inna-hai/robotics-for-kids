...
assert.match(html, /function requestedExerciseFromUrl\(\)[\s\S]*function requestedFreshLessonFromUrl\(\)[\s\S]*params\.get\('v'\) === 'fix-direct-lesson'/, 'direct lesson links support an explicit clean-open mode and the previously shared cache-busted fix link');
assert.match(html, /const lessonChangedByUrl = urlLessonId !== null && savedLessonId !== lessonId/, 'regular lesson links detect when the URL requests a different lesson');
assert.match(html, /const rememberedLessonIndex = lessonChangedByUrl \? firstIncompleteExerciseIndex\(lessonId, exercises\) : Number\(saved\.exercise \|\| 0\)/, 'regular links to another lesson continue from that lesson progress instead of always resetting to exercise 1');
assert.match(html, /if\(saved\.workspaceXml && !directLessonOverride\)/, 'saved Blockly code is restored only for the same active lesson, not copied into a different lesson');
assert.match(html, /if\(freshLessonRequest\) completedExercises\[lessonId\] = new Set\(\);/, 'only explicit reset/fresh links clear completed exercise progress for a lesson');
