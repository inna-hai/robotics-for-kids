...
try {
  ...
  let demoStudent = previewDb.prepare("SELECT id, minecraft_player_name FROM classroom_students WHERE name = 'הדסה בדיקה'").get();
  assert.equal(demoStudent.minecraft_player_name, 'tehila6791692h', 'preview demo student should use Tehila Minecraft player by default');
  previewDb.prepare('UPDATE classroom_students SET minecraft_player_name = ? WHERE id = ?').run('Tehila', demoStudent.id);
  previewDb.close();
  assert.equal((await request(base, '/api/classroom/preview-demo-student-login', { body: {} })).response.status, 200);
  const previewDbUpdated = new Database(dbFile);
  demoStudent = previewDbUpdated.prepare("SELECT id, minecraft_player_name FROM classroom_students WHERE name = 'הדסה בדיקה'").get();
  assert.equal(demoStudent.minecraft_player_name, 'tehila6791692h', 'preview demo login should migrate the old Tehila Minecraft player');
  previewDbUpdated.prepare('UPDATE classroom_students SET minecraft_player_name = ? WHERE id = ?').run('tehila6791692', demoStudent.id);
  previewDbUpdated.close();
  assert.equal((await request(base, '/api/classroom/preview-demo-student-login', { body: {} })).response.status, 200);
  const previewDbHandleOnly = new Database(dbFile);
  demoStudent = previewDbHandleOnly.prepare("SELECT id, minecraft_player_name FROM classroom_students WHERE name = 'הדסה בדיקה'").get();
  assert.equal(demoStudent.minecraft_player_name, 'tehila6791692h', 'preview demo login should migrate the handle-only Tehila Minecraft player');
  previewDbHandleOnly.prepare('UPDATE classroom_students SET minecraft_player_name = ? WHERE id = ?').run('HadasaTest', demoStudent.id);
  previewDbHandleOnly.close();
  assert.equal((await request(base, '/api/classroom/preview-demo-student-login', { body: {} })).response.status, 200);
  const previewDbHadasa = new Database(dbFile);
  demoStudent = previewDbHadasa.prepare("SELECT id, minecraft_player_name FROM classroom_students WHERE name = 'הדסה בדיקה'").get();
  assert.equal(demoStudent.minecraft_player_name, 'tehila6791692h', 'preview demo login should migrate the old HadasaTest Minecraft player');
  previewDbHadasa.close();
  const previewDb2 = new Database(dbFile);
  previewDb2.prepare('UPDATE classroom_teachers SET disabled_at = ? WHERE id = ?').run(new Date().toISOString(), demoTeacher.id);
  previewDb2.close();
  ...
  const previewDb3 = new Database(dbFile);
  previewDb3.prepare('UPDATE classroom_teachers SET disabled_at = NULL WHERE id = ?').run(demoTeacher.id);
  previewDb3.prepare('UPDATE classroom_students SET disabled_at = ? WHERE id = ?').run(new Date().toISOString(), demoStudent.id);
  previewDb3.close();
  assert.equal((await request(base, '/api/classroom/preview-demo-student-login', { body: {} })).response.status, 401);
