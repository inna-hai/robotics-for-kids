...
assert.ok(preview.includes('השיעור הנוכחי - שיעור ${normalizedLessonId}'), 'preview home header labels the current lesson number');
...
assert.ok(academyPage.includes('currentLessonLabel'), 'Agent academy header labels the current lesson dynamically');
assert.ok(academyPage.includes('השיעור הנוכחי - שיעור ${lessonId}'), 'Agent academy header shows current lesson number');
assert.ok(!academyPage.includes('שיעור 0 - מבוך המטבעות'), 'Agent academy header must not show the lesson 0 maze link for later lessons');
...
assert.ok(read('js/craftom-minecraft-lesson-page.js').includes("params.get('teacherReturn') === '1'"), 'teacher-opened student preview can show the Minecraft entry area');
assert.ok(read('js/craftom-minecraft-lesson-page.js').includes('/api/kugel/session?classroomId='), 'teacher-opened student preview checks the live class Minecraft session');
assert.ok(read('js/craftom-minecraft-lesson-page.js').includes('teacherPreviewMinecraft?.launchUrl'), 'teacher-opened student preview can launch Minecraft when the lesson world is available');
assert.ok(read('js/craftom-minecraft-lesson-page.js').includes('/preview-start'), 'teacher-opened student preview refreshes Minecraft access before launching');
assert.ok(read('js/craftom-minecraft-lesson-page.js').includes('function minecraftServerDetails'), 'lesson pages should format Minecraft server details in one place');
assert.ok(read('js/craftom-minecraft-lesson-page.js').includes('קוד שרת: ${serverCode}'), 'lesson Minecraft details should show the server access code');
assert.match(read('js/craftom-minecraft-lesson-page.js'), /שרת: \$\{minecraft\.serverName\}[\s\S]+קוד שרת: \$\{serverCode\}[\s\S]+כתובת: \$\{minecraft\.serverAddress\}/, 'lesson Minecraft details should put the server code immediately after the server name');
...
assert.ok(read('kugel-teacher.html').includes('מצגת מדריך'), 'teacher management page keeps access to instructor slides');
