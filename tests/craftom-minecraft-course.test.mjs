...
assert.ok(preview.includes('20260905-academy-clarity-1'), 'preview page cache-busts the updated challenge data');
...
for (const path of [
  ...
  assert.ok(read(path).includes('20260905-academy-clarity-1'), `${path} loads the updated challenge data`);
}
