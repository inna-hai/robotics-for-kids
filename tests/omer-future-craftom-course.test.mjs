...
assert.ok(hub.includes('poster="${esc(lesson.image)}"'), 'hub uses challenge images only as video posters');
...
assert.ok(!hub.includes('<img class="lesson-thumb"'), 'hub does not duplicate the same challenge image above the video');
...
assert.ok(challengePage.includes('minecraft-shot'), 'challenge page has a styled hero media area');
...
assert.ok(challengePage.includes('challengeVideo.poster = lesson.image'), 'challenge page uses the challenge image only as the video poster');
