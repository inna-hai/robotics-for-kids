...
test('Mission Lab lesson 2 adapts the greenhouse Box Mission as a grade 6 research challenge', () => {
  const lesson = data.TELLO_MISSION_LAB_LESSONS[1];
  assertIncludes(lesson.title, 'חממת העתיד');
  assertIncludes(lesson.concept, 'Pitch');
  assertIncludes(lesson.concept, 'Roll');
  assertIncludes(lesson.concept, 'Yaw');
  assertIncludes(lesson.mission, 'Strafing Box');
  assertIncludes(lesson.mission, 'Yaw Box');
  ...
  assert.ok(lesson.vocabulary.some(([term]) => term.includes('Loop')));
  assert.ok(lesson.teamRoles.length === 3);
  assert.ok(lesson.networkProcedure.some(step => step.includes('WiFi בית ספרי')));
