...
assert.ok(teacher.includes('js/classroom-platform.js?v=20260922-teacher-logout-2'));
assert.ok(!teacher.includes('id="teacher-topbar-logout" type="button" hidden'), 'teacher logout in the topbar should be visible from static HTML');
...
assert.ok(client.includes('students-table'));
assert.ok(client.includes('שם משתמש Minecraft'));
...
assert.ok(!client.includes('add-student-form'), 'teacher-facing student table must not expose student creation controls');
...
assert.ok(styles.includes('.students-table'));
