...
function nextQueuedKugelMinecraftAction(serverName = '') {
  const requestedServer = cleanText(serverName, 80);
  const action = kugelMinecraftActions.find(item => {
    if (item.status !== 'queued' || !item.payload?.command) return false;
    const actionServer = cleanText(item.payload.serverName || item.payload.monitorServerName || '', 80);
    if (requestedServer) return actionServer === requestedServer;
    return !actionServer;
  });
...
function kugelMinecraftInfo(session = null) {
  ...
  return {
    serverName: String(session?.monitor_server_name || '') === 'edu-kugel-holon' ? 'Kugel-Hani' : KUGEL_MINECRAFT_SERVER_NAME,
    serverAddress: `${KUGEL_MINECRAFT_SERVER_HOST}:${KUGEL_MINECRAFT_SERVER_PORT}`,
    serverId: String(session?.monitor_server_name || '') === 'edu-kugel-holon' ? '6ID56SURLMDS' : KUGEL_MINECRAFT_SERVER_ID,
...
function kugelMinecraftInfoForSession(session) {
  if (String(session?.monitor_server_name || '') === 'edu-kugel-sarit') {
    return {
      serverName: 'Kugel-Sarit',
      serverAddress: `${KUGEL_MINECRAFT_SERVER_HOST}:20205`,
      serverId: 'E822HA1HY81D',
      accessCode: KUGEL_MINECRAFT_ACCESS_CODE,
      launchUrl: `minecraftedu://?addExternalServer=${encodeURIComponent('Kugel-Sarit')}|${KUGEL_MINECRAFT_SERVER_HOST}:20205`,
    };
  }
  return kugelMinecraftInfo(session);
}
...
async function kugelClassView(context, role, useEventCache = true) {
  ...
  if (role === 'student') {
    ...
    return {
      ...
      session,
      student: own,
      minecraft: ownsRunningWorld ? kugelMinecraftInfoForSession(data.session) : null,
    };
  }
  return {
    ok: true,
...
function upsertKugelRun(db, studentId, classroomId, patch) {
  ...
  `).run(
    studentId, classroomId, next.started_at, next.reset_at, next.finished_at,
...
async function handleKugelInternalMinecraftApi(req, res) {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/internal/minecraft/actions/next') {
      return send(res, 200, JSON.stringify({ ok: true, action: nextQueuedKugelMinecraftAction(url.searchParams.get('server') || '') }));
    }
    if (req.method !== 'POST') return send(res, 405, JSON.stringify({ error: 'Method not allowed' }));
...
async function handleKugelApi(req, res) {
  ...
  try {
    ...
    const openLomdaMatch = req.method === 'GET' && pathname.match(/^\/api\/kugel\/classes\/([^/]+)\/students\/([^/]+)\/student-view$/);
    if (openLomdaMatch) {
      const context = getTeacherKugelClass(req, decodeURIComponent(openLomdaMatch[1]));
      if (context.status) return send(res, context.status, JSON.stringify({ error: context.error }));
      const studentId = decodeURIComponent(openLomdaMatch[2]);
      const view = await kugelClassView(context, 'teacher', false);
      const own = view.students?.find(s => String(s.id) === String(studentId));
      if (!own) return send(res, 404, JSON.stringify({ error: 'תלמיד לא נמצא.' }));
      return send(res, 200, JSON.stringify({ ok: true, student: own, session: view.session, lesson: view.lesson, classroom: view.classroom }));
    }
    ...
    if (teacherAction) {
      ...
      const activeSession = withSummerDb(db => db.prepare(`
        SELECT classroom_id, monitor_server_name FROM kugel_class_sessions WHERE classroom_id = ? AND active = 1 AND server_state = 'running'
      `).get(classroomId));
      ...
      if (action === 'message') {
        ...
        const payload = {
          classroomId,
          serverName: activeSession.monitor_server_name,
          scope,
    ...
    if (pathname === '/api/kugel/student/start') {
      ...
      return send(res, 200, JSON.stringify({
        ...
        minecraft: kugelMinecraftInfoForSession(state.session),
      }));
    }
