(function () {
      ...
      <header class="course-header" id="courseHeader" aria-label="ניווט אקדמיית ה-Agent">
        <div class="course-header-inner">
          ...
          <nav class="course-nav" id="courseHeaderNav" aria-label="מעבר מהיר">
            ...
            <a class="primary" href="craftom-minecraft-lesson-${lesson.id}.html">השיעור הנוכחי - שיעור ${lesson.id}</a>
  ...
  function minecraftServerDetails(minecraft) {
    if (!minecraft) return 'פרטי השרת יוצגו לאחר שהעולם יהיה זמין.';
  ...
  async function initMinecraftEntry() {
    ...
    const isTeacherPreview = params.get('teacherReturn') === '1';
    const isStudent = me?.role === 'student';
    if (!isStudent && !isTeacherPreview) return;
    ...
    let teacherPreviewMinecraft = null;
    ...
    function render(data) {
      ...
      if (!data.session?.active) {
        ...
        details.textContent = minecraftServerDetails(minecraft);
      }
    ...
    async function refresh() {
      if (!isStudent) {
        const classroomId = params.get('classroomId');
        if (!classroomId) {
          status.textContent = 'בתצוגה מקדימה מהמורה רואים כאן את אזור הכניסה ל-Minecraft כפי שיופיע לתלמיד.';
          details.textContent = 'כדי לפתוח Minecraft מכאן צריך לפתוח את התצוגה מתוך מסך ניהול כיתה.';
          launch.disabled = true;
          teacherPreviewMinecraft = null;
          return;
        }
        try {
          const data = await api(`/api/kugel/session?classroomId=${encodeURIComponent(classroomId)}&lessonId=${encodeURIComponent(String(lesson.id))}`);
          const activeLessonId = Number(data.session?.lessonId ?? 0);
          const activeForThisLesson = Boolean(data.session?.active && activeLessonId === Number(lesson.id));
          teacherPreviewMinecraft = activeForThisLesson ? data.minecraft : null;
          if (!data.session?.active) {
            status.textContent = `המורה עדיין לא פתח את שיעור ${lesson.id} ב-Minecraft.`;
            details.textContent = '';
          } else if (!activeForThisLesson) {
            status.textContent = `פתוח כרגע שיעור ${activeLessonId}. כדי להיכנס לשיעור הזה צריך לפתוח את שיעור ${lesson.id}.`;
            details.textContent = '';
          } else {
            status.textContent = `עולם Minecraft נפתח לשיעור ${lesson.id}. אפשר לפתוח את Minecraft מתוך התצוגה המקדימה.`;
            details.textContent = minecraftServerDetails(teacherPreviewMinecraft);
          }
          launch.disabled = !(activeForThisLesson && teacherPreviewMinecraft?.launchUrl);
        } catch (error) {
    ...
    launch.addEventListener('click', async () => {
      if (!isStudent) {
        if (teacherPreviewMinecraft?.launchUrl) {
          const classroomId = params.get('classroomId');
          if (!classroomId) {
            setMessage('כדי לפתוח Minecraft מכאן צריך לפתוח את התצוגה מתוך מסך ניהול כיתה.', true);
            return;
          }
          setMessage('מרעננים הרשאת כניסה ל-Minecraft…');
          try {
            const data = await api(`/api/kugel/classes/${encodeURIComponent(classroomId)}/lessons/${encodeURIComponent(String(lesson.id))}/preview-start`, {});
            setMessage('Minecraft נפתח. אם האפליקציה לא נפתחה, השתמשו בפרטי השרת שמופיעים כאן.');
            location.href = data.minecraft?.launchUrl || teacherPreviewMinecraft.launchUrl;
            await refresh();
            return;
          } catch (error) {
        ...
        setMessage('Minecraft עדיין לא זמין לשיעור הזה.', true);
        return;
      }
