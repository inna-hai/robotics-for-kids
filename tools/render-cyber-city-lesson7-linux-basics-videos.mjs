...
const FRAME_ROOT = process.env.FRAME_ROOT || '/home/igrois/snap/chromium/common/cyber-city-lesson7-basic-frames';
...
const videos = [
  ['linux-flow', 'פקודה, פלט, ראיה', 'כך עובד טרמינל חקירה', ['כותבים פקודה קצרה.', 'הטרמינל מחזיר פלט.', 'קוראים את הפלט ומבינים מה הוא אומר.', 'כששורה חשודה נדלקת, היא הופכת לראיה.'], ['command', 'output', 'evidence']],
  ['cmd-pwd', 'pwd', 'איפה אני נמצא?', ['הפקודה pwd מציגה את המיקום הנוכחי.', 'במעבדה שלנו הפלט הראשון הוא סימן השורש.', 'לפני שמחפשים ראיות, חשוב לדעת איפה עומדים.'], ['pwd', '/', 'location']],
  ['cmd-ls', 'ls', 'מה יש בתיקייה?', ['הפקודה ls מציגה קבצים ותיקיות.', 'כך רואים שיש תיקיית evidence ותיקיית report.', 'זו מפת הכניסה לתיק החקירה.'], ['ls', 'evidence', 'report']],
  ['cmd-cd', 'cd', 'נכנסים לתיקיית ראיות', ['הפקודה cd עוברת לתיקייה אחרת.', 'כשכותבים cd evidence נכנסים לתיק הראיות.', 'הפרומפט משתנה ומראה שעברנו מקום.'], ['cd evidence', '~/evidence', 'folder']],
  ['cmd-cat', 'cat', 'קוראים קובץ', ['הפקודה cat מציגה את תוכן הקובץ.', 'כך קוראים את login_policy.txt.', 'עכשיו אפשר לראות את חוקי הסיסמה ולא לנחש.'], ['cat', 'login_policy.txt', 'policy']],
  ['cmd-grep', 'grep', 'מחפשים שורה חשודה', ['הפקודה grep מחפשת מילה בתוך קובץ.', 'grep hint מציגה רק את שורת הרמז.', 'grep success מוצאת כניסה שהצליחה בלוג.'], ['grep', 'hint', 'success']]
...
async function createGeminiAudio(video) {
  ...
  for (let index = 0; index < keys.length; index += 1) {
    try {
      console.log(`Creating audio for lesson7 ${video.slug} (${index + 1}/${keys.length})`);
...
<body><main class="frame"><div class="grid"></div><section class="top"><div><div class="kicker">Cyber AI Builders · Lesson 7</div><h1>${esc(video.title)}</h1><h2>${esc(video.subtitle)}</h2></div></section><section class="case"><article class="line"><strong>${esc(line)}</strong><p>פקודה אחת, פלט אחד, הבנה אחת. כך בונים בסיס לינוקס בלי עומס.</p></article><aside class="board">${video.points.map(point=>`<div class="card">${esc(point)}</div>`).join('')}</aside></section><div class="badge">Linux Evidence Basics</div><div class="step">${index + 1}/${video.script.length}</div></main></body></html>`;
}
...
for (const video of videos) {
  ...
  const mp4 = path.join(MARKETING, `cyber-city-lesson7-${video.slug}.mp4`);
  const silentMp4 = path.join(MARKETING, `cyber-city-lesson7-${video.slug}-silent.mp4`);
