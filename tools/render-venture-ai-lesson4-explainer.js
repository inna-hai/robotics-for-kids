...
const OUT_DIR = path.join(MARKETING, 'venture-ai-lesson4-explainer-frames');
const SCRIPT_PATH = path.join(MARKETING, 'venture-ai-lesson4-explainer-script.txt');
const AUDIO_BASE = path.join(MARKETING, 'venture-ai-lesson4-gemini-leda');
const AUDIO_MP3 = `${AUDIO_BASE}.mp3`;
const SILENT_MP4 = path.join(MARKETING, 'venture-ai-lesson4-explainer-silent.mp4');
const OUT_MP4 = path.join(MARKETING, 'venture-ai-lesson4-explainer.mp4');
...
Character: a warm Israeli instructor, energetic and clear, explaining lesson four of an AI entrepreneurship program to school staff and parents.
Style: practical, festive, confident, friendly, not salesy, with natural pauses. Read only the Hebrew lines. Do not read bracket labels.

[warm] זה המפגש האחרון בתוכנית. המטרה שלו היא לסיים את התהליך ולהגיש תוצר שאפשר להציג בגאווה.
[context] לא מתחילים פרויקט חדש, ולא בונים הכול מחדש. לכל צוות כבר יש רעיון מחולון, בריף באופאל, ו-MVP ראשון: אפליקציה, אתר או סוכן קטן שאפשר לפתוח.
[clear] עכשיו הופכים את האב-טיפוס לדמו ברור. מוצר קטן, מובן, ובטוח להגשה ולהצגה.
[peer review] מתחילים בביקורת עמיתים. צוות אחד בודק צוות אחר לפי מחוון: האם הבעיה ברורה, האם יש קשר לחולון, האם הפעולה עובדת, והאם אין מידע אישי.
[opal] אחר כך חוזרים לאופאל לשיפור אחרון. לא אומרים לו לבנות מחדש. בוחרים תיקון חשוב אחד, ועוד שיפור עיצובי אחד.
[focused] למשל: שמור על המבנה, אבל הפוך את כפתור הדיווח לברור יותר. או: הוסף תוצאה אחרי שהמשתמש לוחץ.
[pitch] עכשיו מכינים פיץ׳ של תשעים שניות: מה הבעיה בעיר, למי זה עוזר, מה בנינו, ומה רואים בדמו.
[demo] הצוותים עושים חזרה קצרה. פותחים את הקישור, בודקים שהכול נטען, ומחליטים מי אומר כל חלק.
[day] ואז מגיע Demo Day קטן. כל צוות מציג בעיה, פתרון, דמו, ומה השלב הבא אם היה עוד זמן.
[vote] בסוף אפשר לעשות הצבעת אלף נקודות. לא לפי חברים, אלא לפי המחוון: ברור, שימושי, קשור לחולון, ועובד.
[vision] הסגירה מחברת את כל הדרך: עיר, בעיה, אופאל, בריף, MVP, פיץ׳.
[closing] הילדים מסיימים עם משפט פשוט וגאה: בנינו משהו שעוזר למישהו בחולון לעשות משהו טוב יותר.`;
...
async function render() {
  ...
  await scene({
    title: 'מפגש אחרון',
    subtitle: 'מסיימים את התהליך ומגישים',
    caption: 'המטרה: להגיע לתוצר שאפשר להציג בגאווה.',
    small: 'דמו, פיץ׳ והגשה',
    hero: 3,
    accent: lessons[3].accent,
  }, 8);
  await scene({
    title: 'לא מתחילים מחדש',
    subtitle: 'יש כבר רעיון, בריף ו-MVP ראשון',
    caption: 'המטרה היא להפוך אב-טיפוס לדמו ברור.',
    small: 'אפליקציה, אתר או סוכן קטן שאפשר לפתוח',
    lessons: true,
    flow: true,
    flowOn: 3,
    hotLesson: 3,
  ...
  await scene({
    title: 'ביקורת עמיתים',
    subtitle: 'צוות בודק צוות לפי מחוון',
    caption: 'בעיה ברורה, קשר לחולון, פעולה עובדת, פרטיות והצגה.',
    small: 'פידבק קצר, מדויק ושימושי',
    safety: true,
    accent: '#0891b2',
  }, 11);
  await scene({
    title: 'שיפור אחרון באופאל',
    subtitle: 'תיקון חשוב אחד ושיפור עיצובי אחד',
    caption: 'לא בונים מחדש. משפרים את מה שכבר עובד.',
    small: 'אופאל עוזר לדייק לקראת הדמו',
    opal: true,
    accent: '#14b8a6',
  }, 10);
  await scene({
    title: 'פרומפט תיקון',
    subtitle: 'שמור על המבנה',
    caption: 'הפוך את הפעולה המרכזית לברורה יותר, בלי להוסיף פיצ׳רים.',
    small: 'תיקון קטן, תוצאה גדולה',
    opal: true,
    accent: '#f59e0b',
  }, 8);
  await scene({
    title: 'פיץ׳ 90 שניות',
    subtitle: 'בעיה, משתמש, פתרון, דמו, שלב הבא',
    caption: 'כל צוות יודע מי אומר מה, ומה פותחים במסך.',
    small: 'קצר, ברור, בלי נאום ארוך',
    team: true,
    accent: '#7c3aed',
  }, 10);
  await scene({
    title: 'חזרת דמו',
    subtitle: 'פותחים את הקישור לפני ההצגה',
    caption: 'בודקים שהכול נטען ושהפעולה המרכזית עובדת.',
    small: 'לא מגלים בעיות מול כולם',
    hero: 3,
    accent: '#16a34a',
  }, 9);
  await scene({
    title: 'Demo Day',
    subtitle: 'מציגים תוצר, לא רק רעיון',
    caption: 'בעיה, פתרון, דמו ומה היינו משפרים בהמשך.',
    small: 'זמן קצר לכל צוות',
    outcomes: true,
    accent: '#7c3aed',
  }, 10);
  await scene({
    title: '1000 נקודות',
    subtitle: 'הצבעה לפי מחוון',
    caption: 'ברור, שימושי, קשור לחולון ועובד.',
    small: 'לא לפי חברים. לפי איכות הדמו.',
    lessons: true,
    flow: true,
    flowOn: 3,
    hotLesson: 3,
  ...
  await scene({
    title: 'סגירת מסלול',
    subtitle: 'עיר → בעיה → אופאל → בריף → MVP → פיץ׳',
    caption: 'בנינו משהו שעוזר למישהו בחולון לעשות משהו טוב יותר.',
    small: 'זה הסיום שילדים זוכרים',
    outcomes: true,
    accent: '#16a34a',
  }, 9);
