...
const videos = [
  {
    id: 'brief',
    title: 'פתיחה',
    file: 'cyber-city-lesson2-brief.mp4',
    accent: '#2563eb',
    icon: '01',
    visual: 'מבלוקים לקוד',
    bullets: ['מה הסורק משיעור 1 עשה', 'מה Python יעשה עכשיו', 'איך סימן הופך להחלטה'],
    code: ['signal -> variable', 'if signal:', '    risk += points'],
    script: [
      '[warm] בשלב הראשון אנחנו מחברים את שיעור 1 לקוד.',
      '[clear] בשיעור הקודם מצאנו סימנים בהודעות. עכשיו כל סימן יהפוך למשתנה קצר בפייתון.',
      '[practical] המטרה פשוטה: סימן נכנס לקוד, הקוד מוסיף Risk, ובסוף מקבלים החלטה בטוחה.',
    ],
  },
  {
    id: 'console',
    title: 'Python ראשון',
    file: 'cyber-city-lesson2-console.mp4',
    accent: '#0891b2',
    icon: '▶',
    visual: 'מריצים ורואים פלט',
    bullets: ['ממלאים שני שדות', 'לוחצים על ▶ הרצה מעל הקוד', 'קוראים את הפלט למטה'],
    code: ['print("risk checker")', 'name = input("name: ")', 'print("checking", item)'],
    script: [
      '[friendly] כאן מתנסים בקונסולה הראשונה.',
      '[step by step] קודם ממלאים שם ומה בודקים. אחר כך מסתכלים על חלון הפייתון ולוחצים על כפתור הפליי, הרצה.',
      '[clear] רק אחרי ההרצה מסתכלים באזור פלט. שם רואים מה התוכנית הדפיסה.',
    ],
  },
  {
    id: 'signals',
    title: 'סימנים',
    file: 'cyber-city-lesson2-signals.mp4',
    accent: '#7c3aed',
    icon: '03',
    visual: 'סימן = משתנה',
    bullets: ['בוחרים סימני סיכון', 'רואים שם משתנה קצר באנגלית', 'מבינים כמה נקודות כל סימן מוסיף'],
    code: ['url = True', 'pwd = False', 'code = True', 'fast = True'],
    script: [
      '[focused] בשלב הסימנים אנחנו לא כותבים מילים ארוכות.',
      '[clear] כל סימן מקבל שם קצר באנגלית: url, pwd, code, fast.',
      '[practical] הילד בוחר מה קיים באירוע, והלומדה מראה איך הבחירה נראית בקוד.',
    ],
  },
  {
    id: 'logic',
    title: 'If Lab',
    file: 'cyber-city-lesson2-if-lab.mp4',
    accent: '#16a34a',
    icon: 'if',
    visual: 'בודקים תנאים',
    bullets: ['בוחרים שורות if הגנתיות', 'מריצים אירוע בדיקה', 'רואים Risk ותוצאה בלי לקבל פתרון מלא'],
    code: ['if url:', '    risk += 30', 'if code:', '    risk += 40'],
    script: [
      '[curious] עכשיו מגיע ה־if lab.',
      '[clear] בוחרים רק שורות שעוזרות לסורק הגנתי. לא בוחרים שורה שמבקשת לשלוח סיסמה.',
      '[practical] אחר כך מריצים אירוע ורואים תוצאה: כמה Risk יצא, אילו סימנים הופעלו, ומה מומלץ לעשות.',
    ],
  },
  {
    id: 'debug',
    title: 'Debug',
    file: 'cyber-city-lesson2-debug.mp4',
    accent: '#f59e0b',
    icon: '{}',
    visual: 'מתקנים שגיאות',
    bullets: ['מחפשים מרכאות חסרות', 'בודקים אותיות גדולות וקטנות', 'מחברים קוד סופי שעובד'],
    code: ['print("risk check")', 'if code:', '    risk += 40'],
    script: [
      '[encouraging] בדיבאג לא נבהלים משגיאות. זו עבודה רגילה של מתכנתים.',
      '[clear] מסתכלים על קוד שבור, בוחרים תיקון, ובודקים אם השם של המשתנה והמרכאות נכונים.',
      '[practical] בסוף השלב יש לנו גרסה קצרה שעובדת: מנוע Risk ראשון בפייתון.',
    ],
  },
  {
    id: 'report',
    title: 'דוח סיום',
    file: 'cyber-city-lesson2-report.mp4',
    accent: '#0f766e',
    icon: '✓',
    visual: 'מסבירים את הכלי',
    bullets: ['מה המנוע בודק', 'איך הקוד מחשב Risk', 'מה עושים כשהסיכון גבוה'],
    code: ['Risk Engine v1', 'explain checks', 'choose safe action'],
    script: [
      '[warm] בשלב האחרון לא רק מסיימים קוד. מסבירים אותו.',
      '[clear] כותבים בשלוש שורות מה המנוע בודק, איך ה־if מוסיף ניקוד, ומה עושים כשה־Risk גבוה.',
      '[closing] זה התוצר של השיעור: כלי קטן, וקבוצה שיודעת להסביר למה הוא מגן.',
    ],
  },
];
...
async function createGeminiAudio(video) {
  const base = path.join(MARKETING, `cyber-city-lesson2-${video.id}-leda`);
...
<body>
<div id="stage">
  <div class="brand"><span class="pill">Cyber AI Builders</span><span class="pill light">שיעור 2</span></div>
...
async function renderVideo(browser, video) {
  ...
  const outDir = path.join(MARKETING, `cyber-city-lesson2-${video.id}-frames`);
  const silentPath = path.join(MARKETING, `cyber-city-lesson2-${video.id}-silent.mp4`);
